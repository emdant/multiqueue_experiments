...
/** New Atomics **/
template <typename T, int memorder = __ATOMIC_SEQ_CST>
T atomic_load(T& x) {
  return __atomic_load_n(&x, memorder);
}

template <typename T, int memorder = __ATOMIC_SEQ_CST>
void atomic_store(T& x, T& val) {
  __atomic_store(&x, &val, memorder);
}

template <int memorder = __ATOMIC_SEQ_CST>
void atomic_thread_fence() {
  __atomic_thread_fence(memorder);
}

template <typename T, typename U, int memorder = __ATOMIC_SEQ_CST>
T fetch_and_add(T& x, U inc) {
  return __atomic_fetch_add(&x, inc, memorder);
}

template <typename T, int memorder_success = __ATOMIC_SEQ_CST, int memorder_failure = __ATOMIC_SEQ_CST>
bool compare_and_swap(T& x, T& old_val, T& new_val) {
  return __atomic_compare_exchange(&x, &old_val, &new_val, false, memorder_success, memorder_failure);
}

template <int memorder_success = __ATOMIC_SEQ_CST, int memorder_failure = __ATOMIC_SEQ_CST>
bool compare_and_swap(float& x, float& old_val, float& new_val) {
...
template <int memorder_success = __ATOMIC_SEQ_CST, int memorder_failure = __ATOMIC_SEQ_CST>
bool compare_and_swap(double& x, double& old_val, double& new_val) {
  return __atomic_compare_exchange(
...
//   return __sync_bool_compare_and_swap(reinterpret_cast<uint32_t*>(&x), reinterpret_cast<const uint32_t&>(old_val), reinterpret_cast<const uint32_t&>(new_val));
// }

// template <>
// bool compare_and_swap(double& x, const double& old_val, const double& new_val) {
//   return __sync_bool_compare_and_swap(reinterpret_cast<uint64_t*>(&x), reinterpret_cast<const uint64_t&>(old_val), reinterpret_cast<const uint64_t&>(new_val));
// }
...
bool compare_and_swap(float& x, const float& old_val, const float& new_val) {
  return old_val == atomic_cas_32((volatile uint32_t*)&x, (const volatile uint32_t&)old_val, (const volatile uint32_t&)new_val);
}

bool compare_and_swap(double& x, const double& old_val, const double& new_val) {
  return old_val == atomic_cas_64((volatile uint64_t*)&x, (const volatile uint64_t&)old_val, (const volatile uint64_t&)new_val);
}

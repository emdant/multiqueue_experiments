...
class CSRGraph {
   ...
   public:
    ...
    CSRGraph(int64_t num_nodes, DestID_** out_index, DestID_* out_neighs, DestID_** in_index, DestID_* in_neighs)
        : directed_(true),
          num_nodes_(num_nodes),
    ...
    void PrintStats() const {
        std::clog << "Graph has " << num_nodes_ << " nodes and " << num_edges_ << " ";
        if (!directed_)
            std::clog << "un";
        std::clog << "directed edges for degree: ";
        std::clog << num_edges_ / num_nodes_ << std::endl;
        std::clog << std::endl;
    }
